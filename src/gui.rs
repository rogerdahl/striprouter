...
pub(crate) fn board_to_scr_pos(board_pos: &Pos, zoom: f32, board_screen_offset: &Pos) -> Pos {
    board_pos * zoom + board_screen_offset
}

pub(crate) fn screen_to_board_pos(scr_pos: &Pos, zoom: f32, board_screen_offset: &Pos) -> Pos {
    (scr_pos - board_screen_offset) / zoom
}

pub(crate) fn get_mouse_scr_pos(int_mouse_pos: &Pos) -> Pos {
    int_mouse_pos.cast::<f32>()
}

pub(crate) fn get_mouse_board_pos(int_mouse_pos: &Pos, zoom: f32, board_screen_offset: &Pos) -> Pos {
    screen_to_board_pos(&get_mouse_scr_pos(int_mouse_pos), zoom, board_screen_offset)
}

pub(crate) fn get_component_at_board_pos(circuit: &mut Circuit, board_pos: &Pos) -> Option<String> {
...
pub(crate) fn set_component_position(circuit: &mut Circuit, mouse_board_via: &Pos, component_name: &str) {
    // circuit
